...
def processPostBodyForImages(postBody, imagesPath, imagesUrlPath):
    ...
    tumblrImageUrl = re.compile(
        r"https?://[0-z.]+tumblr\.com/[0-z_/]+(\.jpe?g|\.png|\.gif)"
    )

    while True:

        # Coding pattern recommended by
        # http://docs.python.org/2/faq/design.html#why-can-t-i-use-an-assignment-in-an-expression
        ...
        imageOutputUrlPath = os.path.join(
            imagesUrlPath, imageHash + concreteImageExtension
        )
        ...
        if os.path.exists(concreteImagePath):
            ...
            postBody = postBody.replace(concreteImageUrl, imageOutputUrlPath)
            print(
                "Downloaded image url", concreteImageUrl, "to path", concreteImagePath
            )

    return postBody


def downloader(apiKey, blogName, postsPath, downloadImages, imagesPath, imagesUrlPath):
    ...
    while processed < total_posts:
        response = client.posts(blogName, limit=20, offset=processed, filter="raw")
        ...
        print("Processing...")
        for post in posts:
            print("	http://" + blogName + "/post/" + str(post["id"]))
...
def findFileName(path, slug):
    ...
    print(
        "ERROR: Too many clashes trying to create filename " + makeFileName(path, slug)
    )
    exit()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Tumblr to Markdown downloader",
        epilog="""This app downloads all your Tumblr content into Markdown files that
are suitable for processing with Octopress. Optionally also downloads the
images hosted on Tumblr and replaces their URLs with locally hosted
versions.		""",
    )
    ...
    parser.add_argument(
        ...
        dest="blogName",
        required=True,
        help="Tumblr site host, e.g example.tumblr.com",
    )
    parser.add_argument(
        "--posts-path",
        dest="postsPath",
        default="",
        help="Output path for posts, by default to blogName",
    )
    parser.add_argument(
        "--download-images",
        dest="downloadImages",
        action="store_true",
        help="Whether to download images named on Tumblr into a local folder, and replace their URLs in posts",
    )
    parser.add_argument(
        "--images-path",
        dest="imagesPath",
